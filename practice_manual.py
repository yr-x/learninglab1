...
try: 
    ...
    # nums.append(int(nums[0] / nums[1])) # 오류 발생 시키기 
...
# 에러 발생시키기, 사용자정의 예외처리, finally (무조건실행)
class BigNimberError(Exception):
...
try: 
    ...
    if num1 >= 10 or num2 >= 10:
        raise BigNimberError("입력값: {0}, {1}".format(num1, num2))
    print("{0} / {1} = {2}".forat(num1, num2, int(num / num2)))
except ValueError: 
    print("잘못된 값을 입력하였습니다. 한자리 숫자만 입력하세요") 
except BigNimberError as err:
...
# Q9. 동네에 항상 대기 손님이 있는 치킨집이 있습니다.
# 대기 손님의 치킨 요리 시간을 줄이고자 자동 주문 시스템을 제작합니다
# 시스템 코드를 확인하고 적절한 예외처리 구문을 넣으시오

# 조건1: 1보다 작거나 숫자가 아닌 입력값이 들어올때는 ValueError 처리
#       출력메시지: 잘못된 값을 입력하였습니다
# 조건2: 대기 손님이 주문할수 있는 총 치킨양은 10마리 한정
...
# 모듈 
import theater_model
theater_model.price(3) # 3명 영화 가격
theater_model.price_morning(4) # 조조 
theater_model.price_soldier(5) # 군인

import theater_model as mv # 모듈 별명 
mv.price(3)

from theater_model import * 
price(3)
price_morning(4)
price_soldier(5)

from theater_model import price, price_morning 
price(5)
price_morning(3)

from theater_model import price_soldier as price 
price(5)

# 패키지 
import travel.thailand # 클래스 import 안 됨 
